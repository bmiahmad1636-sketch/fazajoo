...
router.patch('/cases/:id/archive',async(req,res)=>{try{res.json({ok:true,case:await legal.archiveCase(req.user.id,req.params.id)});}catch(e){res.status(e.status||500).json({ok:false,message:e.message});}});
